describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    console.log('Checking all product items');
    for (var i = 0; i < prodItemsData.length; i++){
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    if (!productItem) {
      throw new Error('Product item not found on the page.');
    }
    const shadow = await productItem.getProperty('shadowRoot');
    const button = await shadow.$('button');
    if (!button) {
      throw new Error('Button not found in product item shadowRoot.');
    }

    await button.asElement().click();
    const buttonInnerText = await button.evaluate(element => element.innerText);
    console.log(`Button text after click: ${buttonInnerText}`);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const text = await page.evaluate(count => count.innerText, cartCount);
    expect(text).toBe('20');
  }, 25000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++){
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const button = await shadow.$('button');
      const buttonText = await button.evaluate(element => element.innerText);
      if (buttonText != 'Remove from Cart') allButtonsCorrect = false;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    const prodItems = await page.$$('product-item');
    var allButtonsCorrect = true;
    for (let i = 0; i < prodItems.length; i++){
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const button = await shadow.$('button');
      const buttonText = await button.evaluate(element => element.innerText);
      if (buttonText != 'Add to Cart') allButtonsCorrect = false;
      expect(allButtonsCorrect).toBe(true);
    }
    const cartCount = await page.$('#cart-count');
    const text = await page.evaluate(count => count.innerText, cartCount);
